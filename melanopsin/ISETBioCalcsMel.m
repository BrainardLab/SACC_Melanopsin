...
%{
clear; close all;
conditionNameList = {'MelDirected1', 'MelDirected2', 'MelDirected3', 'MelDirected4', 'MelDirected5'}; 
sineFreqCyclesPerDegList = [1];
gaborSdDeg = 100;
stimulusSizeDeg = 4;
stimConeEccDeg = 5;
...
mosaicOutputStr = sprintf('%s_StimConeEcc_%0.1f_EccX_%0.1f_EccY_%0.1f_FieldSize_%0.1f_AO_%d_NoLCA_%d_Age_%d_macAdj_%0.1f_lensAdj_%0.1f', ...
    conditionName,stimConeEccDeg,eccXDeg,eccYDeg,mosaicSizeDeg,aoRender,noLCA,ageISETBio,dMacISETBio,dLensISETBio);
...
if (aoRender)
    if (eccVars)
        theConeMosaic = ConeResponseCmosaic(eccXDeg, eccYDeg, ...
            'fovealDegree', mosaicSizeDeg, 'pupilSize', pupilDiamMM, 'useRandomSeed', randSeed, ...
...
xlim([eccXDeg-mosaicSizeDeg*0.75/2 eccXDeg+mosaicSizeDeg*0.75/2]);
...
xlim([eccXDeg-mosaicSizeDeg*0.7/2 eccXDeg+mosaicSizeDeg*0.7/2]);
ylim([-10 10]);
...
roiCircle = regionOfInterest('shape','ellipse',...
    ...
    'majorAxisDiameter',mosaicSizeDeg*0.7,...
    'minorAxisDiameter',mosaicSizeDeg*0.7);
...
save(mosaicOutputFilename, ...
    ...
    'T_cones_ISETBio', 'colorDirectionParams', 'eccXDeg', 'eccYDeg', 'mosaicSizeDeg', ...
